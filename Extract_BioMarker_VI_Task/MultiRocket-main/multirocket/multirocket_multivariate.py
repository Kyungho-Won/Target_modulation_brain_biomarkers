            ...
            for kernel_index in range(num_kernels):
                ...
                if _padding1 == 0:
                    for feature_count in range(num_features_this_dilation):
                        ...
                        # end = end + num_features
                        # features[example_index, end] = np.mean(C)
                        # end = end + num_features
                        # features[example_index, end] = np.std(C)


                        # temp = sum(np.power(C-np.mean(C),3))/len(C)
                        # skew = temp/np.power(np.std(C),3)
                        # temp = sum(np.power(C - np.mean(C), 4)) / len(C)
                        # kurtosis = temp / np.power(np.std(C), 4)
                        #
                        # end = end + num_features
                        # features[example_index, end] = skew
                        # end = end + num_features
                        # features[example_index, end] = kurtosis

                        # m_ap = 3
                        # r_ap = 3
                        # end = end + num_features
                        # a = ApEn(C, m_ap, r_ap)
                        # features[example_index, end] = a
                    ...
                    for feature_count in range(num_features_this_dilation):
                        ...
                        features[example_index, end] = mean_index / ppv if ppv > 0 else -1
                        # end = end + num_features
                        # features[example_index, end] = np.mean(_c)
                        # end = end + num_features
                        # features[example_index, end] = np.std(_c)
                        #
                        # temp = sum(np.power(_c - np.mean(_c), 3)) / len(_c)
                        # skew = temp / np.power(np.std(_c), 3)
                        # temp = sum(np.power(_c - np.mean(_c), 4)) / len(_c)
                        # kurtosis = temp / np.power(np.std(_c), 4)
                        #
                        # end = end + num_features
                        # features[example_index, end] = skew
                        # end = end + num_features
                        ...
                        # r_ap = 3
                        # end = end + num_features
                        # features[example_index, end] = ApEn(_c, m_ap, r_ap)
