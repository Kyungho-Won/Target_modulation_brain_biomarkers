...
if __name__ == '__main__':
    ...
    output_path = os.getcwd() + "/output_loocv_0223/"
        ...
        for task in task_list:
            ...
            for sub in sub_str:
                ...
                if os.path.exists(data_folder):
                    ...
                    if isloocv:
                        print(np.shape(total_data)[0] - np.shape(temp_data)[0])
                        print(np.shape(total_data)[0])
                        total_index.append((np.shape(total_data)[0] - np.shape(temp_data)[0] ,np.shape(total_data)[0]))
            ...
            print(total_index)
            for sub in sub_str:
                temp_idx = np.arange(0,np.shape(total_data)[0])
                test_idx = np.arange(total_index[index][0],total_index[index][1])
                train_idx = [ x for x in temp_idx if x not in test_idx]

                X_train = total_data[train_idx,...]
                X_test = total_data[total_index[index][0]:total_index[index][1], ...]
                y_train = total_label[train_idx]
                y_test = total_label[total_index[index][0]:total_index[index][1]]
                ...
                if isloocv:
                    sio.savemat(output_dir + 'loocv/'+ sub + '_classfied.mat',
                                {'predicted_label': yhat_test, 'true_label': y_test})
                else:
                    sio.savemat(output_dir + 'non-loocv/' + sub + '_classwise_classfied.mat',
                                {'predicted_label': yhat_test, 'true_label': y_test})
                ...
                if save:
                    if isloocv:
                        df_metrics.to_csv(output_dir +'loocv/'+ sub +'_results.csv', index=False)
                    else:
                        df_metrics.to_csv(output_dir + 'non-loocv/'+ sub +'_classwise_results.csv', index=False)