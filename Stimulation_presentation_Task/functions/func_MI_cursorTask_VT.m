function func_MI_cursorTask_VT(Params)
...
fp_serial = serialport(VT_port,9600);
...
fname_phase = sprintf('C:/Users/BioComput_HGKim/Desktop/khwon/Stim_for_MI/dat/Phase_%d-%d-%d-%d-%d.mat', time_vec(2), time_vec(3), time_vec(4), ...
...
% default parameters
% Params = struct('nbtrial', 40, 't_cross', 1.5, ...
...
% stim parameters
len_window = 500; % in ms
slides = 50; % in ms
params_stim = struct('freq', [8 13], 'frame', len_window);

cur_fig = squeezed_fig('WindowState', 'fullscreen', 'MenuBar', 'none', 'ToolBar', 'none');
image(imgs_task{8});
...
for nTrial=1:Params.nbtrial
    ...
    whole_class = cat(1, whole_class, current_class-1);
...
%         toc;
        timeLog3D = cat(1, timeLog3D, timeLog);
        ...
        switch current_class % contralateral signal
            case 2 % left
                eeg = double(signalLog(6, :)); % C4
            case 3 % right
                eeg = double(signalLog(2, :)); % C3
        end
        eeg_window = cat(3, eeg_window, double(signalLog));
        cur_phase = tactile_stim_on_phase(eeg, fs, params_stim, 'off');
        whole_phase = cat(1, whole_phase, cur_phase.phase);
        
        if (~isRunning)
            % falling phase
            if cur_phase.phase >= 0
                tic_Running = tic;
                write(fp_serial, current_class-1+'0', "char"); % write 1 or 2 (left or right)
                stim_phase = cat(1, stim_phase, cur_phase.phase);
                isRunning = true;
            end
        end
        
        if ~isempty(tic_Running) && toc(tic_Running) >= 0.1
            isRunning = false;
            tic_Running = [];
        end
...
%     toc;
    ...
    draw_blank(cur_fig, imgs_task{8});
...
out.params = Params;
out.eeg = eeg_window;
out.class = whole_class;
out.whole_phase = whole_phase;
out.stim_phase = stim_phase;
out.timeLog3D = timeLog3D;

save(fname_phase, '-struct', 'out');
close gcf
clear t fp_serial;
fclose('all');
echotcpip('off');
...
function [timeLog, signalLog] = read_dat_DSI(t, timeLog_in, buffer, buffer_size)
...
while notDone
    ...
    if all(ismember(headerStart,data)) % Checks if the packet contains the header
        ...
        if packetType == 1
            ...
            if isempty(timeLog_in)
                timeLog_in = Timestamp;
                buffer = EEGdata';
            else
                timeLog_in = [timeLog_in, Timestamp];
                buffer = [buffer, EEGdata'];
            end
            
            % Limit the size of the logs to buffer_size data points
            if length(timeLog_in) >= buffer_size
                timeLog = timeLog_in;
                signalLog = buffer;
                break;
            end
...
function draw_blank(cur_fig, img)
image(img);
axis off;
set(cur_fig, 'Color', 'k');
end
