...
function func_MI_cursorTask_debug(Params)
disp(Params);
...
nHit = 0; % the number of hits

% load images for MI cursor task
imgs_task = load_imgs_task(); 

% default parameters
% Params = struct('nbtrial', 4, 't_cross', 1.5, ...
...
for nTrial=1:Params.nbtrial
    ...
    if strcmpi(Params.online, 'on')
        ...
        if sum([double(current_instruction), double(OV_dat_cast(end))]) == 2 || sum([double(current_instruction), double(OV_dat_cast(end))]) == 3
            nHit = nHit + 1;
        end
    ...
    cur_axes = draw_blank(cur_fig, imgs_task{8});
    end_draw_blank = toc(begin_draw_blank);
    img_handle = get(cur_axes, 'Children'); 
...
disp(['The number of hit is ', num2str(nHit)]);
send_to_OV(toOV, 31); % send end flag
...
function out = draw_blank(cur_fig, img)
image(img);
axis off;
set(cur_fig, 'Color', 'k');
